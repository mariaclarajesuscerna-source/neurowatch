    ...
    <div className="flex flex-col gap-3.5 px-5 pt-3.5 md:pt-6 md:max-w-lg md:mx-auto">
      ...
      {!bleData.connected && (
        ...
        >
          ...
          <span className="text-base font-bold">Conectar reloj</span>
        </button>
      )}
      ...
      {bleError && (
        <div className="rounded-[14px] bg-alert-fill border border-alert-border p-3.5">
          <p className="text-[13px] font-medium text-alert">{bleError}</p>
        </div>
      )}

      {/* Hero Status — only shown when connected and BPM > 0 */}
      {bleData.connected && bleData.bpm > 0 && <HeroStatus state={status} />}
      ...
      <DeviceCard
        ...
        signalStatus={bleData.connected ? "Senal estable" : "Sin conectar"}
